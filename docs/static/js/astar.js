...
(function() {
    ...
    class AStarSolver {
        ...
        solve(grid, options = {}) {
            ...
            while (!openSet.isEmpty()) {
                ...
                // Compute the path from start to current node for the parent chain highlight
                const parentChain = this._reconstructPath(cameFrom, cx, cy);

                // Record expansion
                steps.push({
    type: STEP_EXPAND,
    current: { x: cx, y: cy, g: cg, h: ch, f: cf },
    currentPath: this._getCurrentPath(cameFrom, cx, cy), // 👈 ADD THIS
    ...
    metrics: { nodesExpanded: metrics.nodesExpanded, nodesGenerated: metrics.nodesGenerated, pathLength: null, pathCost: null }
        ...
        solveDFS(grid, options = {}) {
            ...
            while (stack.length > 0) {
                ...
                steps.push({
                    ...
                    currentPath: this._getCurrentPath(cameFrom, cx, cy),
                    parentChain,
        ...
        solveGraph(graph, options = {}) {
            ...
            while (!openSet.isEmpty()) {
                ...
                // Path from start to current node for parent chain highlight
                const parentChain = this._reconstructGraphPath(cameFrom, cId);
    ...
    class UIController {
        ...
        renderStep(step, path = [], goal = null) {
            ...
            for (const node of step.currentPath) {
    drawNode(node.x, node.y, "current-path"); // different color
}
            // Apply step state
            if (step) {
                // Closed list
            ...
            if (step) {
                ...
                // Uses _reconstructPath output stored in step.parentChain.
                // Highlights the best-known path from start to current node
                // in purple so you can trace the algorithm's progress.
                // Skips start/goal/wall cells; does not override current.
                const showChain = document.getElementById('show-parent-chain')?.checked;
                if (showChain && step.parentChain && step.parentChain.length > 1) {
                    // parentChain[0] is start, parentChain[last] is current node —
                    // skip both endpoints so we only colour the intermediate nodes
                    for (let i = 1; i < step.parentChain.length - 1; i++) {
                        const node = step.parentChain[i];
        ...
        drawHeuristicLine(current, goal, hValue) {
            ...
            if (this.currentHeuristic === 'manhattan') {
                ...
                arrowAngle = Math.PI / 2 * Math.sign(end.py - start.py) || Math.PI / 2;
                if (end.py === start.py) {
    ...
    class GraphUIController {
        ...
        initGraph() {
            ...
            defs.innerHTML = `
                ...
                <marker id="arrowhead-frontier" markerWidth="6" markerHeight="4" refX="5" refY="2" orient="auto">
                    <polygon points="0 0, 6 2, 0 4" fill="#2b271b" />
                </marker>
                <marker id="arrowhead-chain" markerWidth="6" markerHeight="4" refX="5" refY="2" orient="auto">
                    <polygon points="0 0, 6 2, 0 4" fill="#6f42c1" />
                </marker>
            `;
        ...
        renderStep(step, path, goalId) {
            ...
            this.nodeElements.forEach((elem, id) => {
                elem.circle.classList.remove('open', 'closed', 'current', 'path', 'parent-chain');
            });

            // Reset all edge states
            this.edgeElements.forEach(elem => {
                elem.line.classList.remove('path-edge', 'chain-edge');
            ...
            // ── Parent chain highlight (graph mode) ────────────────────────
            // Colours the best-known path from start to the current node purple.
            // Uses _reconstructGraphPath output stored in step.parentChain.
            // Skip start (index 0) and current node (last index).
            const showChain = document.getElementById('show-parent-chain')?.checked;
            if (showChain && step.parentChain && step.parentChain.length > 1) {
                for (let i = 1; i < step.parentChain.length - 1; i++) {
                    const node = step.parentChain[i];
                    const elem = this.nodeElements.get(node.id);
                    if (elem
                        && node.id !== this.graph.start
                        && node.id !== this.graph.goal
                        && !elem.circle.classList.contains('current')) {
                        elem.circle.classList.add('parent-chain');
                    }
                }
                // Also highlight the chain edges
                for (let i = 0; i < step.parentChain.length - 1; i++) {
                    const edgeKey = `${step.parentChain[i].id}->${step.parentChain[i + 1].id}`;
                    const edgeElem = this.edgeElements.get(edgeKey);
                    if (edgeElem && !edgeElem.line.classList.contains('path-edge')) {
                        edgeElem.line.classList.add('chain-edge');
                        edgeElem.line.setAttribute('marker-end', 'url(#arrowhead-chain)');
                    }
                }
            }
