#!/usr/bin/env python3
"""
Generate training prediction data for Cross-Entropy Loss visualization.

This script trains a deliberately weak model on MNIST to show gradual
convergence of predictions over many epochs.

Output: static/json/cross-entropy-training.json
"""
...
NUM_SAMPLES = 10  # One per digit class
SNAPSHOT_EPOCHS = [0, 1, 2, 3, 4, 5, 7, 10, 15, 20, 25]
TOTAL_EPOCHS = 25
BATCH_SIZE = 128
LEARNING_RATE = 0.01  # Higher LR for SGD
TRAIN_SUBSET_SIZE = 10000  # Use 10k samples
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# Output path
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_PATH = os.path.join(SCRIPT_DIR, '..', 'static', 'json', 'cross-entropy-training.json')


class TinyMLP(nn.Module):
    """
    Deliberately weak model - tiny MLP with no convolutions.
    This will learn slowly and show interesting prediction evolution.
    """
...
def image_to_base64(img_array):
    """Convert numpy array (28x28) to base64 PNG string."""
...
def main():
    ...
    print(f"Training with handicaps:")
    print(f"  - Tiny MLP (784 -> 32 -> 10)")
    ...
    # Select samples to track (one per digit class) - from full dataset for variety
    ...
    optimizer = optim.SGD(model.parameters(), lr=LEARNING_RATE)  # SGD is slower than Adam
    ...
    for sample in sample_data:
        ...
        print(f"  Digit {sample['trueLabel']}: "
              f"Loss {first_loss:.3f} → {last_loss:.3f}, "
              f"Final prediction: {last_pred} "
              f"({'correct' if last_pred == sample['trueLabel'] else 'WRONG'})")
